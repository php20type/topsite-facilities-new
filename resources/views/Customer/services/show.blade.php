            ...
            <ul class="list-inline">
                @foreach ($services as $serviceItem)
                    <li class="{{ $service->id == $serviceItem->id ? 'active' : '' }}">
                        <a
                            href="{{ route('user.service.show', ['property' => $property->id, 'service' => $serviceItem->id]) }}">
                            {{ $serviceItem->name }}
                        </a>
                    </li>
                @endforeach
                    ...
                    <div class="head-chat-message-user">
                        <div class="user-box-left-fix">
                            ...
                            <div class="user-info">
                                ...
                                <h6 id="last_seen">Last online 5 hours ago</h6>
                            </div>
                        </div>
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    ...
                                    @elseif ($status === 'In Progress')
                                        <li></li>
                                    @elseif ($status === 'Ready for Review')
                                        <li><a class="dropdown-item" href="#" onclick="updateServiceStatus('Done')">Done</a></li>
                                        <li><a class="dropdown-item" href="#" onclick="updateServiceStatus('New')">New</a></li>
                                    @elseif ($status === 'Done')
                                        <li><a class="dropdown-item" href="#" onclick="updateServiceStatus('New')">New</a></li>
                                    @endif
                                </ul>
...
@section('page_scripts')
    <script>
        ...
        function updateServiceStatus(newStatus) {
            $.ajax({
                url: '/customer-update-service-status',
                method: 'POST',
                data: {
                    status: newStatus,
        ...
        function updateDropdownMenu(newStatus) {
            ...
            if (newStatus === 'In Progress') {
                ...
                $('.dropdown-menu').append('<li><a class="dropdown-item" href="#" onclick="updateServiceStatus(\'Done\')">Done</a></li>');
                $('.dropdown-menu').append('<li><a class="dropdown-item" href="#" onclick="updateServiceStatus(\'New\')">New</a></li>');
        ...
        /* websocket chat module script */
            ...
            function loadChatForLoggedInUser() {
                ...
                // Call the function to make the chat area for the logged-in user
            ...
            conn.onmessage = function(e) {
                ...
                if (data.image_link) {
                    ...
                    var imagePath = '{{ Storage::url('services_images/') }}' + data.image_link;
                    document.getElementById('message_area').innerHTML =
                        `<img src="${imagePath}" class="img-thumbnail img-fluid" />`;
                }
                    ...
                    for (var count = 0; count < online_status_icon.length; count++) {
                        if (online_status_icon[count].id == 'status_' + data.id) {
                            if (data.status == 'Online') {
                                ...
                                document.getElementById('last_seen').innerHTML = 'Online';
                            } else {
