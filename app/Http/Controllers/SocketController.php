...
class SocketController extends Controller implements MessageComponentInterface
{
    ...
    {
        //* to store JSON object data (similar to array[]), but does not accept duplicate objects
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        //*  Proxy object representing a connection to the application, it acts as a container for storing data (in memory) about that connection
        //! add new connection data
        $this->clients->attach($conn); //* add object $conn (store connection_id) to $this->client
        //! populate variable with the request URI from the established connection object, and convert it to a string
        $querystring = $conn->httpRequest->getUri()->getQuery(); //* return URI of established connection as string
        parse_str($querystring, $queryarray); //! convert string to array

        if (isset($queryarray['token'])) {
            User::where('token', $queryarray['token'])->update([
                //! fill the connection_id column with $->resourceId
            ...
            $data['status'] = 'Online';
            //* loop to send data to connected users
    ...
    //* called when a message is sent, receiving data from the client
    public function onMessage(ConnectionInterface $conn, $msg)
    {
        if (preg_match('~[^\x20-\x7E\t\r\n]~', $msg) > 0) {
            //receiver image in binary string message
            $image_name = time() . '.jpg';
            $path = storage_path('app/public/services_images/') . $image_name;
        ...
        $data = json_decode($msg); //* receive data from client and convert to array
        if (isset($data->type)) {
            if ($data->type == 'request_load_unconnected_user') {
                //* get list of users to be chatted with
                ...
                foreach ($this->clients as $client) {
                    //* check if websocket connection (= means authenticated user) == sender's user connection
            ...
            if ($data->type == 'request_search_user') {
                //* get list of users to be chatted with based on input
                $user_data = User::select('id', 'name', 'user_status', 'user_image')
                    ->where('id', '!=', $data->from_user_id)
                ...
                foreach ($user_data as $row) {
                    //* retrieve data from chat_request sender OR receiver
                ...
                foreach ($this->clients as $client) {
                    //* check if connection matches sender's user connection
                    if ($client->resourceId == $sender_connection_id[0]->connection_id) {
            ...
            if ($data->type == 'request_load_unread_notification') {
                ...
                */
                ...
                foreach ($notification_data as $row) {
                    ...
                    if ($row->from_user_id == $data->user_id) { //* akan ada notif di user yg ngirim request
                        //! set notifnya
            ...
            if ($data->type == 'request_send_message') {
                ...
                $chat->service_id = $data->service_id;
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        //! search user in database using connection id and set his/her status offline
        ...
        if (isset($queryarray['token'])) {
            ...
            //! prepare json to update user status to offline in connected users
